resource "aws_elb" "f5-autoscale-ltm-elb" {
  name = "f5-autoscale-ltm-elb-${var.emailidsan}"
...
resource "aws_cloudformation_stack" "f5-autoscaleLTM" {
  ...
  parameters {
    #DEPLOYMENT
    deploymentName           = "autoscale-ltm-${var.emailidsan}"
    vpc                      = "${aws_vpc.terraform-vpc.id}"
    availabilityZones        = "us-east-1d,us-east-1e"
    subnets                  = "${aws_subnet.public-d.id},${aws_subnet.public-e.id}"
    restrictedSrcAddress     = "0.0.0.0/0"
    bigipElasticLoadBalancer = "${aws_elb.f5-autoscale-ltm-elb.name}"
